...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // This prevents the listener from being set up before the initial session check
    const {
      data: { subscription },
      ...
      if (session) {
        ...
        if (typeof window !== "undefined") {
          localStorage.setItem("isLoggedIn", "true")
          localStorage.setItem("userId", session.user.id)
          // Set a session timestamp to track session age
        ...
        // Only clear if we're sure the session is gone
        if (event === "SIGNED_OUT") {
          setUser(null)
          setIsAuthenticated(false)
    ...
    // Set up a periodic session check to prevent unexpected logouts
